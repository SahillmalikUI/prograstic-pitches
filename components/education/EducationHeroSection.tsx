...
import { ArrowRight, Sparkles, ShieldCheck, Activity, CheckCircle2, Zap, Layers, Lock, GraduationCap, Radio, PlaneTakeoff, Globe2 } from "lucide-react";
...
export function EducationHeroSection({ vertical, onOpenScopeModal }: EducationHeroSectionProps) {
  const marqueeBadges = [
    { text: "UCAS Direct University API", tag: "UK ADMISSIONS", icon: <GraduationCap className="w-3.5 h-3.5 text-amber-600" /> },
    { text: "IELTS / PTE Direct TRF Verification", tag: "TEST SCORE", icon: <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> },
    { text: "UKVI / CAS Sponsorship Gateway", tag: "VISA DIRECT", icon: <PlaneTakeoff className="w-3.5 h-3.5 text-blue-600" /> },
    { text: "Canadian DLI & LOA Portal", tag: "CANADA", icon: <Globe2 className="w-3.5 h-3.5 text-purple-600" /> },
    { text: "Sub-Agent Split Commission Ledger", tag: "PAYOUTS", icon: <Zap className="w-3.5 h-3.5 text-rose-600" /> },
  ];
  ...
  return (
    <section className="relative pt-10 pb-20 sm:pb-28 px-4 sm:px-6 lg:px-8 bg-white text-slate-900 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-12 sm:space-y-16 relative z-10">
        {/* Top Scramble & Admissions Telemetry Badges */}
        <motion.div
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="tracking-widest uppercase font-bold text-slate-900">PROGRASTIC EDUCATION OS</span>
            <span className="text-slate-300">•</span>
            <span className="text-amber-700 font-bold bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-200 flex items-center gap-1.5 shadow-sm">
              <Radio className="w-3 h-3 text-amber-600 animate-spin" />
              <span>ADMISSIONS GATEWAY: LIVE</span>
            </span>
          </div>

          <div className="flex items-center gap-6 text-xs font-mono text-slate-600">
            <div>
              <span className="text-slate-400">OFFER VELOCITY: </span>
              <strong className="text-amber-700 font-bold bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
                <AnimatedCounter value={48} suffix=" HRS AVG" />
              </strong>
            </div>
            <div className="hidden sm:block">
              <span className="text-slate-400">VISA GRANT RATE: </span>
              <strong className="text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                <AnimatedCounter value={99.4} suffix="% SUCCESS" decimals={1} />
              </strong>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-6">
          <motion.div
            ...
            className="text-sm sm:text-base font-mono text-amber-700 font-bold uppercase tracking-widest flex items-center gap-2"
          >
            <GraduationCap className="w-4 h-4 text-amber-600 animate-bounce" />
            <span>Study Abroad & Overseas Admissions Platform</span>
          </motion.div>
          ...
          >
            STUDY ABROAD OS &<br />
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-slate-950 via-slate-800 to-amber-600">
              STUDENT DOSSIER.
            </span>
          </motion.h1>
          ...
          >
            <p className="lg:col-span-8 text-xl sm:text-2xl text-slate-600 font-sans leading-relaxed">
              From IELTS verification to 48-hour unconditional university offer letters and visa grant lodgement. Custom software engineered for study abroad agencies.
            </p>
        ...
        {/* Daylight Campus Visual Display with 3D Tilt Physics */}
        <TiltCard className="rounded-[36px] sm:rounded-[48px] overflow-hidden border border-slate-200 bg-slate-50 shadow-2xl group">
          <div className="relative w-full aspect-[16/9] min-h-[380px] sm:min-h-[520px]">
            <Image
              src="/mockups/education_feature.jpg"
              alt="Global University Campus Student Admissions Interface"
              fill
            ...
            <div className="absolute bottom-6 left-6 right-6 flex flex-col sm:flex-row items-stretch sm:items-end justify-between gap-4">
              ...
              <div className="p-5 rounded-2xl bg-white/95 backdrop-blur-xl border border-slate-200 text-slate-900 space-y-1.5 max-w-sm shadow-xl animate-float-slow">
                <div className="flex items-center justify-between text-xs font-mono text-amber-700 font-bold">
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
                    DOSSIER: UNIV OF EDINBURGH
                  </span>
                ...
                <div className="text-sm font-bold font-sans text-slate-900">
                  Unconditional MSc Artificial Intelligence offer letter generated in <AnimatedCounter value={48} suffix=" hrs" />.
                </div>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-white/95 backdrop-blur-xl border border-slate-200 text-slate-900 space-y-1.5 max-w-sm shadow-xl animate-float-delayed">
                <div className="flex items-center justify-between text-xs font-mono text-slate-600 font-bold">
                  <span className="text-blue-700">UKVI STUDENT VISA</span>
                  <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                    APPROVED
                  </span>
                </div>
                <div className="text-sm font-bold font-sans text-slate-900">
                  CAS statement matched with financial proof. Visa granted with zero queries.
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-2 border-t border-slate-200/80 space-y-3">
          <div className="text-[11px] font-mono text-slate-400 uppercase tracking-widest text-center">
            GLOBAL UNIVERSITY PROTOCOLS & VISA CLEARANCE INTEGRATIONS
          </div>
