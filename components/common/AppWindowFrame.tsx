...
export function AppWindowFrame({ title, slug, mockupImage, children }: AppWindowFrameProps) {
  ...
  return (
    <div className="w-full rounded-[28px] sm:rounded-[32px] bg-white border border-slate-200/90 shadow-2xl shadow-slate-900/8 overflow-hidden transition-all">
      {/* macOS / Safari Window Chrome Header */}
      <div className="bg-slate-100/80 backdrop-blur-md px-4 sm:px-6 py-3.5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="hidden md:inline-block text-[11px] font-mono text-slate-500 ml-2">
        ...
        <div className="flex items-center justify-center flex-1 max-w-md mx-auto w-full">
          <div className="w-full bg-white border border-slate-200/90 rounded-full px-3.5 py-1 text-[11px] font-mono text-slate-600 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-1.5 truncate">
              <Lock className="w-3 h-3 text-emerald-600 shrink-0" />
              <span className="text-slate-400">https://</span>
              <span className="text-slate-900 font-semibold truncate">core.prograstic.com/{slug}-os</span>
            </div>
            <span className="hidden sm:inline-flex text-[9px] bg-emerald-50 text-emerald-700 font-bold px-1.5 py-0.2 rounded border border-emerald-200">
              SSL Verified
            </span>
          </div>
        </div>
        ...
        {/* View Mode Toggle */}
        <div className="flex items-center gap-1 self-end sm:self-auto bg-slate-200/70 p-0.5 rounded-full text-xs font-semibold">
          <button
        ...
        {viewMode === "interactive" ? (
          ...
          <div className="rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm space-y-3 animate-in fade-in">
            ...
            <div className="p-4 flex items-center justify-between text-xs text-slate-600 border-t border-slate-100">
              ...
              <span className="font-mono text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
